...
export async function routeTextMessage(ctx: MyContext) {
  ...
  switch (ctx.session.step) {
    ...
    case 'awaiting_elder_phone':
      if (ctx.session.draftElder.hasTelegram) {
        ...
        if (!/^\+\d{8,15}$/.test(phone)) {
          await ctx.reply(
            'Номер должен быть в формате +491234567890. Попробуй ещё раз.'
          )
          return
        }
    ...
    case 'awaiting_med_time':
      ...
      return
    case 'awaiting_med_duration':
      await handleMedDurationInput(ctx, text)
      return
    ...
    default:
      ...
      await ctx.reply(
        'Не понял команду. Используй /help, чтобы увидеть список доступных команд.'
      )
      return
  }
}
