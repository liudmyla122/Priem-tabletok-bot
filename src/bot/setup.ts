...
import { myPlanCommand, handlePlanForSelection } from './commands/myPlan'
import {
  accessibilityCommand,
  handleAccessibilityMode,
} from './commands/accessibility'
import { handleVoiceConfirmation } from './callbacks/voiceConfirmation'

import {
  handleElderHasTelegramYes,
  handleElderHasTelegramNo,
} from './callbacks/elderFlow'
...
} from './callbacks/confirmation'
import {
  handlePreCheckoutQuery,
  handleSuccessfulPayment,
} from './callbacks/payments'
...
export function setupBot() {
  ...
  bot.command('my_plan', myPlanCommand)
  bot.command('accessibility', accessibilityCommand)
  ...
  bot.callbackQuery(/^plan_for_.+$/, handlePlanForSelection)
  bot.callbackQuery(/^access_mode_.+$/, handleAccessibilityMode)
  ...
  // === Голосовое подтверждение (для незрячих, режим BLIND) ===
  bot.on('message:voice', handleVoiceConfirmation)
