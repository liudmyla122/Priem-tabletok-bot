...
export async function triggerMedicationReminder(medicationId: string) {
  ...
  if (!med) {
    // Лекарство удалено — убираем осиротевшую повторяющуюся задачу, чтобы она больше не срабатывала
    await removeMedicationJob(medicationId)
    return
  }
  if (!med.active) {
    // Лекарство отключено — убираем повторяющуюся задачу
    await removeMedicationJob(med.id)
    return
  }

  // Курс завершён (endDate прошёл) — деактивируем и снимаем повторяющуюся задачу
...
async function deliverMedicationReminder(reminderEventId: string) {
  ...
  if (elder.channel === ContactChannel.TELEGRAM && elder.telegramId) {
    ...
    await sendReminderSms(
      elder.phone,
      buildMedicationSmsText(med.name, med.dosage ?? undefined)
    )
  }
...
async function deliverAppointmentReminder(reminderEventId: string) {
  ...
  if (elder.channel === ContactChannel.TELEGRAM && elder.telegramId) {
    ...
    await sendReminderSms(
      elder.phone,
      buildAppointmentSmsText(appt.title, whenText)
    )
  }
...
export async function confirmLatestPendingReminderByPhone(
  phone: string
): Promise<boolean> {
...
/** Находит последнее неподтверждённое напоминание для пользователя по Telegram ID и подтверждает его.
 * Используется для голосового подтверждения незрячими пользователями (режим BLIND). */
export async function confirmLatestPendingReminderByTelegramId(
  telegramId: string
): Promise<boolean> {
  ...
  const pending = await prisma.reminderEvent.findFirst({
    where: {
      confirmedAt: null,
      OR: [
        { medication: { elderId: user.id } },
        { appointment: { elderId: user.id } },
      ],
    },
    orderBy: { createdAt: 'desc' },
  })
...
export async function confirmReminderEvent(
  ...
  const itemLabel = event.medication
    ? `лекарство "${event.medication.name}"`
    : `визит "${event.appointment?.title}"`
...
export async function checkAndEscalateReminder(
  reminderEventId: string
): Promise<void> {
  ...
  const buttonText = event.medication
