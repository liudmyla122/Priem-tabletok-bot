...
export async function helpCommand(ctx: MyContext) {
  await ctx.reply(
    `<b>Справка</b>\n\n` +
      ...
      `2️⃣ Добавь лекарство командой /add_medication — укажи название, дозировку, время приёма и длительность курса (сколько дней принимать). В конце каждого напоминания виден счётчик: день курса и сколько дней уже подтверждено.\n\n` +
      ...
      `5️⃣ Команда /my_plan показывает полный план приёма всех лекарств на сегодня с прогрессом по каждому.\n\n` +
      `6️⃣ Если тебе или подопечному сложно пользоваться обычным интерфейсом — команда /accessibility включает режимы для незрячих (голосовые напоминания и голосовое подтверждение) и для глухих (визуально-акцентные напоминания без звука).`,
    { parse_mode: 'HTML' }
  )
}
