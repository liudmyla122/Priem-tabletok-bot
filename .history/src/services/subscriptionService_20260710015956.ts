...
export async function checkAccess(userId: string): Promise<AccessCheckResult> {
  ...
  if (sub.status === SubscriptionStatus.ACTIVE) {
    if (sub.currentPeriodEnd && sub.currentPeriodEnd < now) {
      ...
      // [DEBUG] истёкшая ПЛАТНАЯ подписка возвращает reason=TRIAL_EXPIRED (неверно)
      console.log(
        `[DEBUG checkAccess] ACTIVE sub истёк (currentPeriodEnd=${sub.currentPeriodEnd.toISOString()}) -> возвращаем reason=TRIAL_EXPIRED (должно быть подписка истекла)`
      );
